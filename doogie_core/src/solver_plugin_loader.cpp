...
boost::shared_ptr<doogie_core::BaseSolver> SolverPluginLoader::getSolverInstance(){
  setSolverNameFromParams();
  getSolverInstance(solver_name_);
  return solver_;
}

boost::shared_ptr<doogie_core::BaseSolver> SolverPluginLoader::getSolverInstance(const std::string& solver_name){
  try
  {
